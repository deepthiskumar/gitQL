...
%A regular expression is a sequence of characters which acts as a syntax for the matching texts. 
...
\textbf{The Choice calculus}

[Cite paper, Eric's Ph.D. thesis, GTTSE tutorial]
...
Consider the example of two different implementations of a function \texttt{twice} in C.
...
Consider a C program \textit{P} which consists of a function \texttt{f}. This is the first version of the program. %No changes have been made to it and therefore the variational program \textit{VP} is same as \textit{P}.
...
A programmer makes the following sequence of edits on \texttt{f}. After each edit, choices are added in the variational program VP.
...
\begin{lstlisting}[caption=Encoding of version history of a C program file, label=lst:enc1, gobble=0, basicstyle=\ttfamily\small]
... 
...
<pattern> ::= <character>  
            ...
            \alt <pattern> <pattern>
            \alt <pattern> `|' <pattern>
            \alt <dimension> `<' <pattern> `,' <pattern> `>'
            \alt \textdollar <string>
            \alt `(' <pattern> `)'
...
<dimension>    ::= <integer> \alt <string> \alt $\epsilon$
